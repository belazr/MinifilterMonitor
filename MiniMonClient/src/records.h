...
namespace mimo {
    ...
    namespace records {
        ...
        inline std::optional<std::span<const protocol::Record>> Parse(const AlignedBuffer& buffer) {
            constexpr uint32_t RECORD_SIZE = static_cast<uint32_t>(sizeof(protocol::Record));

            if (buffer.Size() % RECORD_SIZE != 0u) return std::nullopt;

            const protocol::Record* const pRecords = reinterpret_cast<const protocol::Record*>(buffer.Data());

            return std::span<const protocol::Record>{ pRecords, buffer.Size() / RECORD_SIZE };
        }
